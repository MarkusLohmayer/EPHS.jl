...
function grid_dimensions(pattern::Pattern{F,Position}) where {F}
  ...
  foreachvalue(pattern.junctions) do (_, (;r, c))
    r_min = min(r, r_min)
    r_max = max(r, r_max)
    c_min = min(c, c_min)
    c_max = max(c, c_max)
  ...
  foreachvalue(pattern.boxes) do (_, (;r, c))
    r_min = min(r, r_min)
    r_max = max(r, r_max)
    c_min = min(c, c_min)
    c_max = max(c, c_max)
...
function draw_ports(io, pattern, scale)
  ...
  foreachvalue(pattern.boxes) do (box, bpos)
    foreachvalue(box.ports) do port
      (_, jpos) = pattern.junctions[port.junction]
...
function draw_external_ports(io, pattern, scale, grid_dims, dims)
  ...
  foreachvalue(pattern.junctions) do (junction, jpos)
    if junction.exposed
      (x2, y2) = scale .* (jpos.c, jpos.r)
      dist_c_min = jpos.c - c_min
...
function draw_boxes(io, pattern, scale)
  foreach(pattern.boxes) do (path, (box, (;r, c)))
    (x, y) = scale .* (c, r)
    color = box.filling isa AbstractSystem ? fillcolor(box.filling) : "white"
    print(io,
      """
...
function draw_box_name(io, path)
  words = split(string(path), '_')
